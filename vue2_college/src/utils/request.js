...
const request = axios.create({
    ...
    transformResponse: [function (data) {
        ...
        try {
          ...
          // console.log('转换失败', err)  不是json格式就会打印
          // 如果转换失败了，则进入这里
  ...
  if (status === 400) {
    ...
    try {
      const { data } = await refreshTokenReq({
        url: '/app/v1_0/authorizations', // 刷新用户token的接口       
      })
...
// 替换当初路由路径 不会形成历史记录
  router.replace({
    ...
    query: {
      // 数据名是自己起的     
      // router.currentRoute 和我们在组件中获取的 this.$route 是一个东西(当前路径)
